...
class SimpleEngine(Model):
    ...
    def setup(self):
        ...
        c = self.c = Variable("c", 1766, "m/s", "effective exhaust speed of engine")

        F = self.F = Variable("F", 1000, "N", "Engine thrust")

        OF = self.OF = Variable("OF", 7.5, "", "Ox to fuel ratio")
        ...
        d = self.d = Variable("d_ox", 6, "in", "Diameter of ox tank")
        P_ox = self.P = Variable("Tank P", 60, "bar", "Max Ox Tank pressure")
        sigma_max = Variable("\sigma_{max}", 585, "MPa", "Max stress of tank, Al-7075-T6")

        # determine the wall thickness needed
        SF = Variable("SF", 3, "", "Wall thickness safety factor")
        ...
        rho_ox = Variable("rho_{ox}", 650, "kg/m^3", "density of liquid ox ROUGH")
        ...
        rho_tank = Variable("rho_{ox, tank}", 8000, "kg/m^3", "Density of ox tank (if al)")
        ...
        constraints += [Tight([A_grain <= 0.25*3.14*(d/2)**2])]
        ...
        # impose bounds
        constraints += [Loose([l_ox >= 0.1 * ureg.m, l_ox <= 2 * ureg.m])]
